...
class UserUpdateProfilePictureScreen extends StatefulWidget {
  final UserModel? loggedInUser;
  const UserUpdateProfilePictureScreen({super.key, this.loggedInUser});

  @override
  State<UserUpdateProfilePictureScreen> createState() =>
      _UserUpdateProfilePictureScreenState();
}

class _UserUpdateProfilePictureScreenState
    extends State<UserUpdateProfilePictureScreen> {
  File? imageFile;
  ...
  Widget build(BuildContext context) {
    ...
    void _validateSelectedImages() {
      if (profilePictureFile != null) {
        ...
        BlocProvider.of<UserUpdateProfilePicBloc>(context).add(
            UserUpdateProfilePicEventStarted(imageFile: profilePictureFile!));
      } else {
          ...
          child:
            ...
            if (profilePictureFile != null)
              Padding(
                ...
                child: BlocConsumer<UserUpdateProfilePicBloc,
                    UserUpdateProfilePicState>(
                  listener: (context, state) {
                    if (state is UserUpdateProfilePicSuccessState) {
                      EasyLoading.showSuccess('Profile Picture Updated');
                      Future.delayed(Duration(seconds: 2), () {
                        context.pop();
                      });
                    } else if (state is UserUpdateProfilePicErrorState) {
                      EasyLoading.showError(state.message);
                    }
                  },
                  builder: (context, state) {
                    if (state is UserUpdateProfilePicLoadingState) {
